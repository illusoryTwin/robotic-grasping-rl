"""Training script for two-stage grasping task using RSL-RL PPO.

This script trains a UR10 manipulator with a two-stage approach:
...
Usage:
    ...
    python scripts/train_two_stage_grasp.py --task Isaac-TwoStageGrasp-UR10-v0

    # Train with custom number of environments
    python scripts/train_two_stage_grasp.py --task Isaac-TwoStageGrasp-UR10-v0 --num_envs 2048

    # Train in headless mode (no GUI)
    python scripts/train_two_stage_grasp.py --task Isaac-TwoStageGrasp-UR10-v0 --headless --num_envs 4096
"""
...
parser = argparse.ArgumentParser(description="Train two-stage grasp task with RSL-RL PPO")
parser.add_argument("--task", type=str, default="Isaac-TwoStageGrasp-UR10-v0", help="Name of the task.")
...
def main():
    ...
    # Create environment
    print(f"[INFO] Creating two-stage grasp environment...")
    ...
    # Start training
    print(f"[INFO] Starting two-stage grasp training for {agent_cfg.max_iterations} iterations")
    print(f"[INFO] Training approach:")
    print(f"       - Stage 1 (RL): Policy predicts 6-DoF grasp pose")
    print(f"       - Stage 2 (IK): Scripted execution via differential IK")
    print(f"[INFO] Reward: Sparse (success only after lift)")
