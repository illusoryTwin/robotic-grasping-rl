...
def grasp_success_sparse(
    env: ManagerBasedRLEnv,
    lift_height: float = 0.1,
    object_cfg: SceneEntityCfg = SceneEntityCfg("object"),
) -> torch.Tensor:
    """Sparse reward for grasp success in two-stage grasping.

    Returns 1.0 if object is lifted above threshold, 0.0 otherwise.
    This reward is designed for the two-stage grasp environment where
    reward is only computed after the EVALUATE phase.

    Args:
        env: The environment instance.
        lift_height: Minimum height above table for successful grasp.
        object_cfg: Configuration for the object entity.

    Returns:
        Sparse reward tensor of shape (num_envs,).
    """
    obj: RigidObject = env.scene[object_cfg.name]
    obj_height = obj.data.root_pos_w[:, 2]

    success = obj_height > lift_height
    return success.float()


def grasp_success_with_stability(
    env: ManagerBasedRLEnv,
    lift_height: float = 0.1,
    max_velocity: float = 0.1,
    object_cfg: SceneEntityCfg = SceneEntityCfg("object"),
) -> torch.Tensor:
    """Sparse reward with stability bonus for two-stage grasping.

    Returns:
        1.0 if object lifted above threshold
        +0.5 bonus if object is stable (low velocity)

    Args:
        env: The environment instance.
        lift_height: Minimum height for success.
        max_velocity: Maximum velocity for stability bonus.
        object_cfg: Configuration for the object entity.
    ...
    lifted = (obj_height > lift_height).float()
    stable = (obj_vel < max_velocity).float()

    return lifted * (1.0 + 0.5 * stable)