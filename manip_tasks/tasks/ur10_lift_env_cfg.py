...
from manip_tasks.rewards import both_fingers_contact_soft, penalize_non_finger_contact, object_centered_between_fingers
...
class RewardsCfg:
    ...
    # Start with higher weight, curriculum reduces to 1.0 at step 1500
    reaching_object = RewTerm(func=object_ee_distance, params={"std": 0.5}, weight=5.0)

    # Start with weight 0, curriculum increases to 15.0 at step 1500
    lifting_object = RewTerm(func=object_is_lifted, params={"minimal_height": 0.05}, weight=0.0)
    ...
    # Encourage object COM to be centered on finger axis
    object_centered = RewTerm(
        func=object_centered_between_fingers,
        params={"std": 0.02}, # 0.05},
        weight=10.0,
    )

    # # Penalize contact with non-gripper parts
...
class CurriculumCfg:
    ...
    # At step 1500: lifting reward activates (0 -> 15)
    lifting_object = CurrTerm(
        func=mdp.modify_reward_weight, params={"term_name": "lifting_object", "weight": 15.0, "num_steps": 1500}
    )

    # At step 1500: reaching reward decreases (5 -> 1)
    reaching_object = CurrTerm(
        func=mdp.modify_reward_weight, params={"term_name": "reaching_object", "weight": 1.0, "num_steps": 1500}
    )
...
#     # # Reset robot to vertical grasp pose above object
#     # reset_robot_vertical_grasp = EventTerm(
#     #     func=reset_robot_to_vertical_grasp_pose,
#     #     mode="reset",
#     #     params={
#     #         "vertical_height_range": (0.1, 0.15),  # 10-15cm above object (matching z_offset)
#     #         "horizontal_offset_range": (-0.02, 0.02),  # Small random offset
#     #     },
#     # )
